...
def create_virtualenv(venv_folder_path):
    venv.create(venv_folder_path)

def activate_virtualenv(venv_folder_path):
    system_os = platform.system()
    if system_os == "Windows":
        activate_script = os.path.join(os.path.abspath(venv_folder_path), "Scripts", "activate.bat")
    else:
...
def install_requirements(venv_folder_path, requirements_file_path):
    activate_virtualenv(venv_folder_path)
